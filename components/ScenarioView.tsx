    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="bg-slate-900 rounded-3xl overflow-hidden border border-slate-800 shadow-2xl">
        ...
        <div className="p-8 md:p-12">
          <div className="prose prose-invert max-w-none prose-headings:text-blue-400 prose-p:text-slate-300">
            {scenario?.split('Resolution')[0].split('\n').map((line, idx) => (
              <p key={idx} className="whitespace-pre-wrap">{line}</p>
            ))}
          </div>
          <div className="mt-12 border-t border-slate-800 pt-8">
            <button onClick={() => setShowResolution(!showResolution)} className="flex items-center gap-3 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all w-full md:w-auto">
      ...
      <div className="flex justify-center">
        <button onClick={fetchAiScenario} className="text-slate-500 hover:text-blue-600 font-medium text-sm flex items-center gap-2">
